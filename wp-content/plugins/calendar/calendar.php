 ...
 * @return mixed
 **/
function fetchUpcomingEvents($cat_list = '')
{
    global $wpdb;
    ...
    if ($display == 'true') {
        ...
        while ($day_count < $future_days+1) {
            ...
            usort($events, "time_cmp");
            if (count($events) != 0) {
                $events_output[$day_count]['date'] = date_i18n(get_option('date_format'),mktime($day_count*24,0,0,date("m",ctwo()),date("d",ctwo()),date("Y",ctwo())));
            }
            ...
            foreach($events as $event) {
                ...
                $events_output[$day_count]['events'][] = draw_event($event).$time_string;
            }
    ...
    return !empty($events_output) ? $events_output : false;
}

function upcomingEventsHomePage($lines=NULL)
{
    $e = fetchUpcomingEvents();

    //format contents here
    $lines = isset($lines) ? $lines : 0;
    $counter = 1;
    $has_events = false;
    if (!empty($e)) {
        $has_events = true;
        $cal_output .= '<table class="table-full-width">';
        $cal_output .= '<tr><td>';
        foreach($e as $d) {
            $cal_output .= '<div class="module-block-event">';
            $cal_output .= '<table>';
            $cal_output .= '<tr><td><h3 class="entry-title">' . $d['date'] . '</h2></td></tr>';
            $counter++;
            foreach ($d['events'] as $ee) {
                $cal_output .= '<tr><td>' . $ee . '</td></tr>';
                $counter++;
    ...
    return array('content' => $cal_output, 'has_events' => $has_events);
}
